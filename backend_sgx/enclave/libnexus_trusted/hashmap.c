...
__find_entry_ptr(const struct hashmap * map, const struct hashmap_entry * key)
{
	struct hashmap_entry **e = &map->table[bucket(map, key)];
	while (*e && *e != key)
		e = &(*e)->next;
...
static void *
do_remove(struct hashmap * map, struct hashmap_entry ** e)
{
	struct hashmap_entry *old = *e;
	*e = old->next;
	old->next = NULL;
...
void *
__hashmap_remove_entry(struct hashmap * map, struct hashmap_entry * entry)
{
	struct hashmap_entry **e = __find_entry_ptr(map, entry);
	if (!*e)
		return NULL;

	/* remove existing entry */
	
	return do_remove(map, e);
}
