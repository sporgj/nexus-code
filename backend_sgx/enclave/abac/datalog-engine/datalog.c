...
static int
dl_pcall(dl_db_t L, int nargs, int nresults)
{
    ...
    if (i) {
        ...
        if (s)
            ...
            log_error("no error message available\n");
        lua_pop(L, 1);
    }
    return i;
}
...
dl_lua(dl_db_t L)
{
    int i = luaL_loadbuffer(
        L, (const char *)datalog_lua_bytes, sizeof(datalog_lua_bytes), datalog_lua_source);
    if (i) {
        const char * s = lua_tostring(L, -1);
        if (s)
            log_error("%s\n", s);
...
#if !defined __STDC_VERSION__ || __STDC_VERSION__ < 199901L
    b = (dl_answers_t)malloc(sizeof(struct dl_answers) + (n - 1) * sizeof(char **));
#else
