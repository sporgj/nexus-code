...
static char *
dl_putc(char * ptr, int c, int * left)
{
    switch (c) {
    case '\a':
        c = 'a';
    ...
    default:
        if (isprint(c)) {
            ptr = my_putc(ptr, c, left);
        } else {
            int nchars = snprintf(NULL, 50, "\\%03o", (unsigned char)c);

            if (nchars > *left) {
                snprintf(ptr, 50, "\\%03o", (unsigned char)c);
                *left -= nchars;
            }
...
dl_putlconst(char * ptr, const char * s, size_t n, int * left)
{
    ...
    if (islidentifier(s, n)) {
        while (s < t) {
            ptr = my_putc(ptr, *s++, left);
        }
    } else {
        ptr = my_putc(ptr, '"', left);

        while (s < t) {
            ptr = dl_putc(ptr, *s++, left);
        }
...
DATALOG_API char *
dl_putconst(char * dest_buffer, const char * s, int * left)
{
