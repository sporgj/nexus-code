...
typedef struct {
    ...
    uint8_t input_buffer[CONFIG_CRYPTO_BUFLEN];
...
{
    if (context->sealing_key) {
        free(context->sealing_key);
    }
...
ecall_xfer_init(xfer_context_t * xfer_ctx)
{
    ...
    context->sealing_key
        = derive_skey(&header->root, &header->uuid);
    ...
    if (xfer_ctx->xfer_op == UCAFS_STORE) {
        ...
        enclave_crypto_ekey((crypto_ekey_t *)ekey, context->sealing_key,
                            UC_DECRYPT);
    }
    ...
    /* initialize the context map structure */
