...
TEST_F(TestDirops, FileCreation)
{
    ...
    ASSERT_EQ(0, stat(path2, &stat_buf)) << "Filebox " << path2
                                         << " does not exist";
...
TEST_F(TestDirops, FileRenaming1)
{
    ...
    /* Creating the files */
    ASSERT_EQ(0, dirops_new(path1, type, &temp1)) << "dirops_new failed";
...
TEST_F(TestDirops, HardLinkTest1)
{
    ...
    /* 1 - Create the file */
    ASSERT_EQ(0, dirops_new(filepath, UC_FILE, &temp)) << "dirops_new failed";
...
TEST_F(TestDirops, HardLinkTest3)
{
    ...
    ASSERT_TRUE((filebox1 = dcache_get_filebox(filepath, 0)) != NULL)
        << "Getting the file's filebox failed";

    ASSERT_TRUE((filebox2 = dcache_get_filebox(linkpath, 0)) != NULL)
...
TEST_F(TestDirops, SymLinkTest1)
{
    ...
    ASSERT_FALSE((fb1 = dcache_get_filebox(filepath, 0)) == NULL)
    ...
    ASSERT_FALSE((fb2 = dcache_get_filebox(linkpath, 0)) == NULL)
        << "Link filebox cannot be null";
    ...
    ASSERT_TRUE((fb2 = dcache_get_filebox(linkpath, 0)) == NULL)
...
TEST_F(TestDirops, SymLinkTest2)
{
    ...
    /* 3 - Access the symlink */
    ASSERT_TRUE((fb1 = dcache_get_filebox(linkpath, 0)) == NULL)
        << "Dereferencing cannot lead to null";

    /* 4 - Access the file */
    ASSERT_FALSE((fb2 = dcache_get_filebox(filepath, 0)) == NULL);

    ASSERT_EQ(0, dirops_symlink(dir4_path, link2tget, &temp3));

    ASSERT_FALSE((fb1 = dcache_get_filebox(link2path, 0)) == NULL);
